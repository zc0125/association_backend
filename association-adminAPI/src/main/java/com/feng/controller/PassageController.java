...
public class PassageController {
    ...
    public ResponseResult list(@RequestParam(defaultValue = "1") int pageNum, @RequestParam(defaultValue = "8") int pageSize, Passage search) {
        PageInfo<PassageDto> passageList = passageService.findPage(pageNum, pageSize, search);
        return ResponseResultUtil.renderSuccess(passageList);
    }
    @GetMapping("/club")
    @ApiOperation("根据根据社团Id以及条件分页查询所有文章")
    public ResponseResult ListByClubId(@RequestParam(defaultValue = "1") int pageNum, @RequestParam(defaultValue = "8") int pageSize, Passage search) {
        PageInfo<PassageDto> passageList = passageService.findPageByClubId(pageNum, pageSize, search);
        return ResponseResultUtil.renderSuccess(passageList);
    }
