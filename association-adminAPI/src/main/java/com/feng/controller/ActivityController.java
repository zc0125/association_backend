...
public class ActivityController {
    ...
    public ResponseResult list(@RequestParam(defaultValue = "1") int pageNum, @RequestParam(defaultValue = "8") int pageSize, Activity search) {
        PageInfo<ActivityDto> activityPageInfo = activityService.getPage(pageNum, pageSize, search);
        return ResponseResultUtil.renderSuccess(activityPageInfo);
    }

    @ApiOperation(value = "根据社团及条件分页查询所有活动",notes = "根据条件分页查询所有活动")
    @GetMapping("/club")
    public ResponseResult listByClub(@RequestParam(defaultValue = "1") int pageNum, @RequestParam(defaultValue = "8") int pageSize, Activity search) {
        PageInfo<ActivityDto> activityPageInfo = activityService.getPageByClubId(pageNum, pageSize, search);
        return ResponseResultUtil.renderSuccess(activityPageInfo);
    }
